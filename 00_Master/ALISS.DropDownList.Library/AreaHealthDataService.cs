...
namespace ALISS.DropDownList.Library
{
    public class AreaHealthDataService : IAreaHealthDataService
    {
        ...
        public List<TCAreaHealthContact> Get_AreaHealthContact_List(TCAreaHealthContact searchModel)
        {
            log.MethodStart();
            ...
            using (var trans = _db.Database.BeginTransaction())
            {
                try
                {
                    objList = _db.AreaHealthContactDTOs.FromSqlRaw<TCAreaHealthContact>("sp_GET_DDL_AreaHealthContact {0}", searchModel.arc_arh_code).ToList();
